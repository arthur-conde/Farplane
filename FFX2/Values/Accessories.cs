...
public static class Accessories
{
    ...
    public static List<string> GetNames() => AccessoryList.Select(accessory => accessory.Name).ToList();

    public static Accessory FromName(string accessoryName) => AccessoryList.First(accessory => string.Equals(accessory.Name, accessoryName, StringComparison.CurrentCultureIgnoreCase));

    public static Accessory FromBytes(byte[] accessoryBytes) => AccessoryList.First(accessory => accessory.ID == accessoryBytes[1]);

    public static Accessory FromID(int accessoryID) => AccessoryList.First(accessory => accessory.ID == accessoryID);
